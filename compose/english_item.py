...
class EnglishItem(Item):
    ...
    def export_as_html(self):
        return '\n'.join([self.export_for_english(d, self.key, self.style) for d in self.definitions])

    @staticmethod
    def export_for_english(definition: BaseDefinition, key: str, style: str = DEFAULT_HTML_STYLE) -> str:
        ...
        def_tags = t_d_tags % definition.case_tags.replace("-", "&zwj;-&zwj;") if definition.case_tags else ''
        ...
        word_origin_x = DefinitionFormatter(definition).tagged_word_origin_x(t_word_origin)
