...
from compose import DEFAULT_HTML_STYLE, Item
from compose.definition_formatter import DefinitionFormatter
...
class Meaning(Item):
    ...
    def export_as_html(self) -> str:
        ...
        if self.style == "normal":
            used_in_list = f'<div class="used_in">Used In: ' \
                           f'{used_in}</div>\n</div>' \
                if used_in else "</div>"
            return f'<div class="meaning" id="{mid}">\n' \
    ...
    def generate_meaning(self) -> tuple:
        ...
        html_affixes, html_match, html_rank,\
            ...
            html_year, t_technical = self.get_styled_values()
    ...
    def used_in_as_html(self) -> str:
        ...
        return " |&nbsp;".join(sorted(
            {tags[self.style] % cpx.name for cpx in filter(None, self.word.complexes)}
        ))
    ...
    def get_styled_values(self) -> tuple:
        ...
        values = [self._ewc.e_affixes, self.word.match, self.word.rank, self._ewc.e_source, self.word.type.type,
                  self.used_in_as_html(), self._ewc.e_year, None]
        default_values = [str(), str(), str(), str(), str(), None, str(), tags[self.style][-1]]

        return tuple(self._tagger(tag, value, default_value) for tag, value, default_value
                     in zip(tags[self.style], values, default_values))
...
class LoglanItem(Item):
    ...
    @staticmethod
    def query_select_words(name, case_sensitive: bool = False,
            event_id: Event | int | str = None) -> Select:
        words = WordSelector().by_name(
            name=name,
...
class Composer(Item):
    ...
    def export_as_html(self) -> str:
        ...
        items = [LoglanItem(words_list, self.style).export_as_html() for words_list in iterator]
